  ...
  class Parser
    ...
    def parser(options)
      OptionParser.new do |parser|
        ...
        parser.on('--[no-]fail-fast[=COUNT]',
                  'Abort the run after a certain number of failures (1 by default).') do |argument|
          if argument == true
            value = 1
        ...
        parser.on('-o', '--out FILE',
                  ...
                  '  if no format is specified.') do |o|
        ...
        parser.on('--force-color', '--force-colour',
                  'Force the output to be in color, even if the output is not a TTY') do |_o|
        ...
        parser.on('--no-color', '--no-colour',
                  'Force the output to not be in color, even if the output is a TTY') do |_o|
