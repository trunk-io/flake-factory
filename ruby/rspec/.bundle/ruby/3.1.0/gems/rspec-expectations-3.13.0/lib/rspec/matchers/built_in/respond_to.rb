  ...
  module Matchers
    module BuiltIn
      ...
      class RespondTo < BaseMatcher
        ...
        # @return [String]
        def failure_message
          "expected #{actual_formatted} to respond to #{@failing_method_names.map { |name|
                                                          description_of(name)
                                                        }.join(', ')}#{with_arity}"
        end
        ...
        def matches_arity?(actual, name)
          ArityCheck.new(@expected_arity, @expected_keywords, @arbitrary_keywords, @unlimited_arguments).matches?(
            actual, name
          )
        rescue NameError
